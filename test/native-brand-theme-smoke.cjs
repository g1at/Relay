...
const root=path.resolve(__dirname,'..'),output=path.join(root,'.codex-tmp/native-brand-theme-smoke');
...
app.whenReady().then(async()=>{
  ...
  const source=fs.readFileSync(path.join(root,'main.js'),'utf8'),begin=source.indexOf('function resolveNativeIcon('),end=source.indexOf('// ── 系统托盘',begin);
  const trayImages=[];
  context=vm.createContext({app,process,path,fs,__dirname:root,nativeImage,nativeTheme,BrowserWindow,console,
    tray:{isDestroyed:()=>false,setImage:image=>trayImages.push(image)},
    require:name=>{if(name!=='./native-brand-theme')throw Error('Unexpected fixture import');return require('../native-brand-theme');}});
