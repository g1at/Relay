...
> **3.0.2 安装修复版准备中。** 3.0.1 和本次 3.0.2 修复版均采用本仓库与 `relay-updates` 双源发布；3.0.3 及后续版本只使用本仓库。旧客户端已缓存旧包、无法重新检查时，需要手动运行正式发布的完整修复包。详见 [升级与交付说明](distribution/PUBLISHING.md#已下载旧安装包的客户端)。
