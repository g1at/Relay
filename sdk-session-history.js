...
function executeSessionOperation(scope, operation, input = {}, options = {}) {
  ...
  const script = expandAsar(options.workerPath || path.join(__dirname, 'sdk-session-history-worker.cjs'));
  const sdkPath = expandAsar(options.sdkPath || path.join(__dirname, 'node_modules', '@anthropic-ai', 'claude-agent-sdk', 'sdk.mjs'));
