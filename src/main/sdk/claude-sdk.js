...
const { resourcePath, unpackedPath } = require('../app/paths');
const fs = require('fs');
...
const { createUsageObserver } = require('../usage/usage-capture');
const { LiveAsyncAgentTracker, liveResultDisposition } = require('../live/live-async-agent-tracker');
...
const { LiveMcpReadiness, McpControlQueue } = require('../live/live-mcp-readiness');
const { normalizeExecutionMode, executionToolPolicy, LiveExecutionModes, PLAN_INSTRUCTIONS } = require('../projects/execution-modes');
...
const { taskFileInstructions, createTaskFilePolicy } = require('../tasks/task-file-policy');
...
function loadSdk() {
  if (!sdkPromise) {
    const entry = resourcePath('node_modules', '@anthropic-ai', 'claude-agent-sdk', 'sdk.mjs');
...
function bundledClaudeVersion() {
  try {
    ...
    try {
      const p = resourcePath('node_modules', '@anthropic-ai', 'claude-agent-sdk', 'package.json');
