...
const { appRoot, resourcePath, unpackPath } = require('../app/paths');
const { spawn, execFile } = require('node:child_process');
const { bridgeMcpServers } = require('./host-stdio-mcp');
const { mcpConfigKey } = require('../live/live-mcp-readiness');
...
function runtimePath(base = appRoot) {
  return unpackPath(path.join(base, 'node_modules', '@anthropic-ai', 'claude-agent-sdk-linux-x64', 'claude'));
}
function createAgentEnvironment(options = {}) {
  ...
  async function inspectSettings({ cwd, settingSources = ['user'], wslDistribution } = {}) {
    ...
    try {
      const script = unpackPath(options.diagnosticScript || path.join(__dirname, 'sdk-settings-probe.cjs'));
      const sdk = unpackPath(options.sdkModulePath || resourcePath('node_modules', '@anthropic-ai', 'claude-agent-sdk', 'sdk.mjs'));
