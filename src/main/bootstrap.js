...
const { createSdkRuntimeStorage } = require('./sdk/sdk-runtime-storage');
...
const { createConversationWorkspaces, workspaceKey, directoryValue, conversationContext, UUID: WORKSPACE_UUID } = require('./projects/conversation-workspaces');
const { createProjectStore } = require('./projects/project-store');
const { normalizeExecutionMode, prepareExecutionRequest } = require('./projects/execution-modes');
const { protectGoalRecovery, previousConversationGoal, condition: goalConditionValue } = require('./projects/conversation-goals');
const { registerWorkspaceTools } = require('./workspace/workspace-tools');
const { createGeneralPreferences, registerGeneralPreferencesIpc, normalizePreferences, DEFAULTS: GENERAL_PREFERENCE_DEFAULTS } = require('./app/general-preferences');
const { createAgentEnvironment, toWslPath } = require('./sdk/agent-environment');
const { buildRuntimePolicy, createRuntimeDiagnostics } = require('./sdk/sdk-runtime-policy');
const { loadNativeAgent } = require('./sdk/native-agent-definition');
const { requiresFreshContract, contractFingerprints, migrateLegacyRuntimeContract, RUNTIME_FINGERPRINT_VERSION } = require('./sdk/sdk-runtime-contract');
const { SUPPORTED_DIALOG_KINDS, createUserDialogHandler } = require('./sdk/sdk-user-dialog');
const { safeFindings, nativeWorkingDirectory } = require('./sdk/sdk-native-events');
const { createHistoryManagement } = require('./sdk/sdk-history-management');
const { createPluginStore } = require('./sdk/sdk-plugin-store');
const { createToolProposalHook } = require('./sdk/sdk-tool-proposals');
let sdkPluginStore;
function getSdkPluginStore() { return sdkPluginStore ||= createPluginStore({ file: path.join(app.getPath('userData'), 'sdk-plugins.json') }); }
const { SdkSessionObserver, stopOwnedTask, backgroundOwnedTask, observeOwnedBackgroundTasks, RouteTimingHistory } = require('./sdk/sdk-session-observer');
const { resourceEntries, mergeResources, ownedResource, resourceTarget } = require('./sdk/sdk-task-resources');
const { createMcpPermissions } = require('./sdk/sdk-mcp-permissions');
const { observeProvenance, applyProvenance, protectSdkMetadata, resolveStoredScope, pendingForkOptions } = require('./sdk/sdk-session-provenance');
const { createSessionForkService } = require('./sdk/sdk-session-forks');
const { createSessionHistoryService, executeSessionOperation } = require('./sdk/sdk-session-history');
const { registerNativeAttachmentDialog } = require('./app/native-attachment-dialog');
const { createMiniWindowHost, isQuickChatEnabled, normalizeQuickChatPatch } = require('./app/mini-window-host');
const { createMiniChatController } = require('./app/mini-chat-controller');
const { createTaskbarCompletionBadge, createNativeTaskbarOverlay } = require('./app/taskbar-completion-badge');
const updater = require('./app/updater');
const claudeSdk = require('./sdk/claude-sdk');
const { LiveTurnRouter } = require('./live/live-turn-router');
const { createPrewarmState, canReuseFreshPrewarm } = require('./live/live-prewarm-reuse');
const { TaskClock } = require('./tasks/task-clock');
const { TaskContinuityHost } = require('./tasks/task-continuity-host');
const historyStore = require('./app/history-store').createHistoryStore({
  getUserDataDir: () => app.getPath('userData'),
  recoverLegacyOutput: (...args) => recoverLegacyOutput(...args),
});
const { getHistoryDir, historyIndexPath, convFilePath, writeJsonAtomic, convMeta, readHistoryIndex, writeHistoryIndex, rebuildHistoryIndex, loadConversation, persistConversationRecord, forEachConversation, migrateHistoryV1 } = historyStore;
const taskContinuityHost = new TaskContinuityHost({ loadConversation });
const { normalizeSupplement, sameSupplement, submitLiveSupplement, observeSupplement, flushSupplementUpdates, mergeSupplementHistory } = require('./live/live-supplement-input');
const recoverLegacyOutput = require('./projects/legacy-output-recovery').createLegacyOutputRecovery();
const { LiveTurnControls } = require('./live/live-turn-control');
const { ProviderStore } = require('./providers/provider-store');
const {
  providerApiUrl,
  imageRequestRoute,
} = require('./providers/provider-connectivity');
const { TaskLedger } = require('./tasks/task-ledger');
const { TaskEventJournal, isValidEpoch } = require('./tasks/task-event-journal');
const { TaskProgressClient } = require('./tasks/task-progress-client');
const { TaskOrchestrator } = require('./tasks/task-orchestrator');
const {
  InteractionBroker,
  isAppPermissionMode,
  resolveUnattendedPermissionMode,
} = require('./tasks/interaction-broker');
const { createConversationPermissions, isConversationPermissionMode } = require('./projects/conversation-permissions');
const { CheckpointManager } = require('./tasks/checkpoint-manager');
const { SkillDraftClient } = require('./skills/skill-draft-client');
const { prepareSkillGenerationWorkspace } = require('./skills/skill-generation-workspace');
const { SkillMaintenanceHost } = require('./skills/skill-maintenance-host');
const { normalizeMaintenancePolicy, evaluateMaintenanceRun } = require('./skills/skill-maintenance-policy');
const { normalizeMemoryMeta, memoryEligibility } = require('./memory/memory-schema');
const {
  RUN_STATES,
  RUN_EVENT_SCHEMA_VERSION,
  isTerminalState,
  selectTaskSnapshotRuns,
} = require('./tasks/task-protocol');
const { compactText, normalizeClaudeTaskEvent } = require('./tasks/task-event-normalizer');
const {
  LiveAsyncAgentTracker,
  LiveBackgroundTaskTracker,
  liveResultDisposition,
} = require('./live/live-async-agent-tracker');
const { dispatchLiveInput, cancelPendingLiveInput } = require('./live/live-mcp-dispatch');
...
const { readAppSettings, writeAppSettings, readLegacyClaudeSettings } = require('./app/app-settings-service').createAppSettingsService({
  getUserDataDir: () => app.getPath('userData'), providerStore,
  onWrite: () => { miniBrandCache = null; if (miniHost) publishMiniState(); },
});
...
const { createMemoryRuntime, MEMORY_CONSOLIDATION_PROMPT } = require('./memory/memory-runtime');
...
//   打包后 appRoot 在 app.asar 内,且 build/ 不在 asar 里 —— 旧逻辑只查 appRoot/build/icon.ico
//   必然 false,导致托盘拿到空图标(系统托盘图标空白的根因)。
//   故打包态优先取 extraResources 解包出的真实文件 process.resourcesPath/icon.ico;dev 态用源码 build/。
const applicationWindows = require('./app/application-windows').createApplicationWindows({
  electron: { app, BrowserWindow, ipcMain, shell, Tray, Menu, nativeImage, nativeTheme }, logger,
  settings: { read: readAppSettings, write: writeAppSettings },
  providers: { store: providerStore, publishChange: (...args) => publishProviderChange(...args) },
  tasks: {
    runningCount: () => jobs.size + busyLiveCount(),
    waitingInteractions: () => interactionBroker.list(),
    rejectWindow: (...args) => interactionBroker.rejectWindow(...args),
    refreshBadge: () => taskbarCompletionBadge.refresh(),
    scheduleRetention: () => scheduleStartupTaskLedgerRetention(),
    nextScheduledHint: () => scheduler.nextTaskHint(),
  },
  mini: {
    shortcut: () => registeredMiniAccel, toggle: () => toggleMiniWindow(),
    registerShortcut: () => registerMiniShortcut(), getHost: () => getMiniWindowHost(),
    getExistingHost: () => miniHost,
  },
  browser: { openLink: (...args) => browserPanelTools.openLink(...args) },
  startup: { providerConfigured: () => relayProviderConfigured() },
  IS_DEV, IS_AUTOSTART, HAS_SINGLE_INSTANCE_LOCK,
});
const { currentAppIcon, refreshTrayMenu, showMainWindow, openConfiguredWebLink } = applicationWindows;
...
function emitCheckpointUpdate(checkpoint) {
  ...
  try {
    if (applicationWindows.mainWindow && !applicationWindows.mainWindow.isDestroyed()) {
      applicationWindows.mainWindow.webContents.send('checkpoints:event', { type: 'checkpoint.updated', checkpoint });
    }
  } catch (_) {}
}
...
function emitInteractionChange(event) {
  ...
  try {
    ...
    const target = miniPanel && !miniPanel.isDestroyed() && interaction && String(interaction.windowId) === String(miniPanel.id) ? miniPanel : applicationWindows.mainWindow;
...
const taskbarCompletionBadge = createTaskbarCompletionBadge({
  setOverlay: createNativeTaskbarOverlay({ assetDirectory: path.join(appRoot, 'renderer', 'taskbar-badges'),
    onError: error => console.warn('[taskbar-badge-native] %s', error.message) }),
  getWindow: () => process.platform === 'win32' ? applicationWindows.mainWindow : null,
  imageForCount: count => {
    ...
    if (!taskbarBadgeImages.has(name)) {
      ...
      const directory = path.join(appRoot, 'renderer', 'taskbar-badges');
  ...
  isForeground: run => focusedRelayWindow(applicationWindows.mainWindow)
...
app.on('browser-window-focus', (_event, win) => {
  if (win === applicationWindows.mainWindow) taskbarCompletionBadge.clear();
...
function broadcastTaskEnvelope(envelope) {
  try {
    if (applicationWindows.mainWindow && !applicationWindows.mainWindow.isDestroyed()) applicationWindows.mainWindow.webContents.send('tasks:event', envelope);
...
function scheduleStartupTaskLedgerRetention() {
  ...
  const timer = setTimeout(() => {
    if (applicationWindows.isQuitting) return;
    try {
...
function getMiniChat() {
  if (!miniChat) miniChat = createMiniChatController({
    ...
    onHistoryChanged: id => {
      try { if (applicationWindows.mainWindow && !applicationWindows.mainWindow.isDestroyed()) applicationWindows.mainWindow.webContents.send('mini:history-changed', { id }); } catch (_) {}
    },
  });
  return miniChat;
}
function getMiniWindowHost() {
  if (!miniHost) miniHost = createMiniWindowHost({
    BrowserWindow, screen: require('electron').screen, Menu, rootDir: appRoot,
...
function miniInteractionWindowId(runId) {
  ...
  return applicationWindows.mainWindow && !applicationWindows.mainWindow.isDestroyed() ? applicationWindows.mainWindow.id : null;
}
...
async function openMiniConversationInMain() {
  ...
  showMainWindow();
  if (applicationWindows.mainWindow && miniMainReadySender === applicationWindows.mainWindow.webContents && !applicationWindows.mainWindow.webContents.isLoadingMainFrame()) {
    applicationWindows.mainWindow.webContents.send('mini:open-conversation', { id: pendingMiniConversationId });
...
function cronMcpFactory() {
  ...
  return (sdk) => {
    try {
      const { createCronMcpServer } = require('./scheduling/cron-mcp');
      return {
  ...
  const onNativeHook = async input => {
    ...
    if (!childHook && ['CwdChanged', 'FileChanged'].includes(input.hook_event_name)
      ...
      if (!sess.workspaceRefreshTimer) sess.workspaceRefreshTimer = setTimeout(() => {
        ...
        if (!sess.dead && liveSessions.get(convId) === sess) applicationWindows.mainWindow?.webContents.send('workspace:runtime-changed', { conversationId: convId, root: sess.workspaceRoot || null });
      }, 180);
    }
...
function finishTurn(sess, resultEvt) {
  ...
  if (checkpointManager && jobId && checkpointRecord) {
    checkpointManager.preview(jobId).then((record) => {
      try {
        if (applicationWindows.mainWindow && !applicationWindows.mainWindow.isDestroyed()) {
          applicationWindows.mainWindow.webContents.send('checkpoints:event', { type: 'checkpoint.updated', checkpoint: record });
        }
      } catch (_) {}
    }).catch((e) => {
...
registerGeneralPreferencesIpc({ ipcMain, getWindow: () => applicationWindows.mainWindow, service: generalPreferences, dialog });
...
const workspaceTools = registerWorkspaceTools({
  ipcMain, getWindow: () => applicationWindows.mainWindow, resolveWorkspace: resolveWorkspaceForTools, resolveLinkWorkspace, shell,
...
const browserPanelTools = require('./browser/browser-panel-ipc').registerBrowserPanelIpc({
  ipcMain, getWindow: () => applicationWindows.mainWindow, entryFile: path.join(appRoot, 'renderer/index.html'),
});
const attachmentDialog = registerNativeAttachmentDialog({
  ipcMain, getWindow: () => applicationWindows.mainWindow, dialog,
});
...
ipcMain.handle('sdkPlugins:add', async event => {
  if (!permissionCaller(event)) return { ok: false, code: 'FORBIDDEN' };
  const selected = await dialog.showOpenDialog(applicationWindows.mainWindow, { title: '添加本地 SDK 插件', properties: ['openDirectory'] });
...
ipcMain.handle('checkpoints:preview', async (_e, runId) => {
  ...
  try {
    ...
    try { if (applicationWindows.mainWindow && !applicationWindows.mainWindow.isDestroyed()) applicationWindows.mainWindow.webContents.send('checkpoints:event', { type: 'checkpoint.updated', checkpoint }); } catch (_) {}
...
function broadcastSkillDraftEvent(type, payload) {
  try {
    if (applicationWindows.mainWindow && !applicationWindows.mainWindow.isDestroyed()) {
      applicationWindows.mainWindow.webContents.send('skillDrafts:event', { type, ...payload });
    }
  } catch (_) {}
}
...
require('./skills/skill-draft-ipc').registerSkillDraftIpc({
  ipcMain, getSkillDraftService: () => skillDraftService, broadcastSkillDraftEvent,
...
ipcMain.handle('shell:open', async (event, url) => {
  ...
  if (/^https?:\/\//i.test(url)
    && (applicationWindows.mainWindow && event.sender === applicationWindows.mainWindow.webContents && event.senderFrame === event.sender.mainFrame || miniPanelCaller(event))) {
...
// IPC: 历史会话 CRUD
// ─────────────────────────────────────────
require('./app/history-ipc').registerHistoryIpc({
  ipcMain, history: historyStore, saveConversation, deleteConversation, genId, protectSdkMetadata,
  projects: { initialize: initializeProjectHistory, getStore: getProjectStore, projectConversation },
  nativeHistory: { getManagement: getSdkHistoryManagement },
  isMiniChatActive: id => !!(miniChat && miniChat.isRunning() && miniChat.getConversationId() === id),
});
...
const SKILL_USAGE_WORKER_FILE = app.isPackaged
  ? path.join(process.resourcesPath, 'app.asar.unpacked', 'src', 'main', 'skills', 'skill-usage-worker.js')
  : path.join(appRoot, 'src/main/skills/skill-usage-worker.js');
...
const { UsageStatsService } = require('./usage/usage-stats-service');
...
function getUsageStatsService() {
  if (!usageStatsService) usageStatsService = new UsageStatsService({
    ...
    workerFile: app.isPackaged
      ? path.join(process.resourcesPath, 'app.asar.unpacked', 'src', 'main', 'usage', 'usage-stats-worker.js')
      : path.join(appRoot, 'src/main/usage/usage-stats-worker.js'),
    onUpdated: payload => {
      for (const win of BrowserWindow.getAllWindows()) {
...
function recordRelayUsage(record, source, conversationId = null) {
  if (applicationLifecycle?.isUsageClosed()) return;
...
function permissionCaller(event) {
  return !!event && !!event.sender && event.senderFrame === event.sender.mainFrame
    && ((applicationWindows.mainWindow && !applicationWindows.mainWindow.isDestroyed() && event.sender === applicationWindows.mainWindow.webContents) || miniPanelCaller(event));
}
...
require('./app/settings-ipc').registerSettingsIpc({
  ipcMain, app, providerStore, readAppSettings, writeAppSettings, generalPreferences,
  windows: applicationWindows, applyParallelTaskLimit, publishProviderChange,
  onEnvironmentChanged: () => {
    supportedModelsCache.clear();
    for (const sess of [...liveSessions.values()]) if (!sess.busy) killLiveSession(sess, '智能体运行环境已切换');
  },
  onQuickChatChanged: () => {
    registerMiniShortcut(); getMiniWindowHost().syncSettings(); refreshTrayMenu();
  },
  refreshMiniBrand: () => { miniBrandCache = null; if (miniHost) publishMiniState(); },
});

// ─────────────────────────────────────────
// IPC: Relay 服务商管理
// ─────────────────────────────────────────
require('./providers/provider-ipc').registerProviderIpc({ ipcMain, providerStore, publishProviderChange });
...
function brandProfileCaller(event) {
  return !!applicationWindows.mainWindow && !applicationWindows.mainWindow.isDestroyed() && !!event?.sender
    && event.sender === applicationWindows.mainWindow.webContents && event.senderFrame === event.sender.mainFrame
    && event.senderFrame.url === pathToFileURL(path.join(appRoot, 'renderer/index.html')).href;
}
...
ipcMain.handle('brand:pickLogoPreview', async event => {
  ...
  try {
    const result = await dialog.showOpenDialog(applicationWindows.mainWindow, { title: '选择个人资料头像', properties: ['openFile'], filters: [{ name: '图片', extensions: BRAND_IMG_EXT.map(ext => ext.slice(1)) }] });
...
require('./app/mini-local-images').registerMiniLocalImages({
  ipcMain, isCaller: event => miniPanelCaller(event)
    && event.senderFrame.url === pathToFileURL(path.join(appRoot, 'renderer/mini.html')).href,
...
ipcMain.handle('mini:mainReady', event => {
  if (!applicationWindows.mainWindow || applicationWindows.mainWindow.isDestroyed() || event.sender !== applicationWindows.mainWindow.webContents) return { ok: false };
...
function tickSkillMaintenance() {
  if (skillMaintenanceTickPromise || applicationWindows.isQuitting || !getReviewConfig().autoArchive) return;
  skillMaintenanceTickPromise = (async () => {
    ...
    if (refreshed?.ok !== true || refreshed.complete !== true || applicationWindows.isQuitting) return;
...
require('./memory/memory-ipc').registerMemoryIpc({
...
function initScheduler() {
  try {
    scheduler.init({
      ...
      getMainWindow: () => applicationWindows.mainWindow,    // 推送 sched:update 给 renderer
      ...
      notify: ({ title, body }) => {
        try {
          ...
          } else if (applicationWindows.tray) {
            applicationWindows.tray.displayBalloon({ icon: currentAppIcon() || undefined, title: title || 'Relay', content: body || '' });
          }
        } catch (_) {}
      },
    });
...
async function maybeAskAutostart() {
  ...
  writeAppSettings(a);
  try {
    const r = await dialog.showMessageBox(applicationWindows.mainWindow || undefined, {
      type: 'question',
      buttons: ['开启开机自启', '暂不开启'],
      defaultId: 0,
      cancelId: 1,
...
require('./scheduling/scheduler-ipc').registerSchedulerIpc({
...
// Electron 生命周期
// ─────────────────────────────────────────
applicationLifecycle = require('./app/application-lifecycle').registerApplicationLifecycle({
  app, BrowserWindow, nativeTheme, powerMonitor, Notification, globalShortcut,
  windows: applicationWindows, updater, scheduler, settings: { read: readAppSettings },
  history: { migrateHistoryV1, migrateScheduledTitles },
  taskRuntime: {
    jobs, liveSessions, resourceLeases: taskResourceLeases, interactions: interactionBroker,
    getProgressStore: () => taskProgressStore, getOrchestrator: () => taskOrchestrator,
    getLedger: () => taskLedger, killLiveSession, interruptActiveRuns: interruptActiveShadowRuns,
    flushStreams: flushStreamJournalEvents, flushEvents: flushTaskJournalEvents,
    applyParallelTaskLimit, scheduleRetention: scheduleStartupTaskLedgerRetention,
  },
  mini: {
    getChat: () => miniChat, getHost: () => miniHost,
    getOrCreateHost: getMiniWindowHost, registerShortcut: registerMiniShortcut,
  },
  memory: { hasPendingUsage: () => !!_memoryUsageFlushTimer, flushUsage: flushMemoryUsage },
  integrations: { attachmentDialog, browserPanelTools, workspaceTools },
  initializeScheduler: initScheduler, getSkillDraftService: () => skillDraftService,
  getUsageStatsService, getExistingUsageStatsService: () => usageStatsService,
  HAS_SINGLE_INSTANCE_LOCK,
});
